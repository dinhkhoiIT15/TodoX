...
const TaskCard = ({ task, index, handleTaskChanged }) => {
  ...
  const updateTask = async () => {
    try{
      ...
      toast.success("Cap nhat nhiem vu thanh cong");
      handleTaskChanged();
    } catch (error) {
  ...
  const toggleTaskCompleteButton = async () => {
    try{
      if(task.status === 'active') {
        ...
        toast.success(`${task.title} da duoc hoan thanh`);
        handleTaskChanged();
      } else {
        await api.put(
          `/tasks/${task._id}`,
      ...
      toast.error("Loi xay ra khi cap nhat trang thai nhiem vu");
    }
  }
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          {isEditing ? (
            <Input
              ...
              className="flex-1 h-12 text-base border/50 focus:border-primary/50 focus:ring-primary/20"
              type="text"
        ...
        <div className="hidden gap-2 group-hover:inline-flex animate-slide-up">
          ...
          onClick={() => {setIsEditing(true); setUpdateTaskTitle(task.title || '')}}
          >
